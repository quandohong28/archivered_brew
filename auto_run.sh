...
if [ "$LOCAL_INSTALL" == "true" ]; then
  LOCAL_FILE="install_bash/${MACOS_VERSION}.sh"
  ...
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/$INSTALLER_HASH/install.sh)"
fi
...
if ! brew doctor; then
  log_warn "Some issues detected by brew doctor. Please review above logs."
fi

# ----- Untap local core -----
log_info "Untapping Homebrew/core..."
brew untap homebrew/core >/dev/null 2>&1 || true
